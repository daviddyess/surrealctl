...
program
  .version('1.0.0')
  .description('CLI for managing SurrealDB instances');
...
program
  ...
  .action((options) => {
    checkIfRunning((isRunning) => {
      if (isRunning) {
        log('SurrealDB is already running. Use start --force to start anyway.', 'warning');
        return;
      }
      
      const config = readConfig();
      const args = ['start', '-A', config.dataDir, '--user', config.username, '--pass', config.password];
      ...
      log(`SurrealDB instance started with PID ${surrealProcess.pid}`, 'success');
    });
  });
  ...
  .action((options) => {
    checkIfRunning((isRunning) => {
      if (isRunning && !options.force) {
        log('SurrealDB is already running. Use --force to start anyway.', 'warning');
        return;
      }
...
function readConfig() {
  if (!fs.existsSync(CONFIG_FILE)) {
    return { dataDir: process.env?.SURREAL_PATH ?? './data', username: process.env?.SURREAL_USER ?? 'root', password: process.env?.SURREAL_PASS ?? 'root' };
  }
    ...
    exec(`ps -p ${pid} -o comm=`, (error, stdout, stderr) => {
      if (error) {
        ...
        if (processName.includes('surreal')) {
          ...
          log(`Found process (PID: ${pid}) is not a SurrealDB instance. Cleaning up.`, 'warning');
          fs.unlinkSync(PID_FILE);
          callback(false);
        }
      }
...
program.on('--help', () => {
  ...
  console.log('  $ surrealdb-cli configure -d ./my-data -u myuser -p mypassword');
